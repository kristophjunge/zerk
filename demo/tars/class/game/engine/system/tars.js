 ...
 * Tars System
 * 
 * Provides tars controls for jump and run style games.
 * 
 * @class tars
...
zerk.define({
	...
	name: 'tars.game.engine.system.tars',
	...
	init: function(engine,config) {
		...
		zerk.parent('tars.game.engine.system.tars').init.apply(
			this,
			arguments
		);
		...
		if (!this._physics) {
			...
			console.log('Tars system need physics system to be active');
	...
	addEntity: function(entity) {
		...
		zerk.parent('tars.game.engine.system.tars').addEntity.apply(
			this,
			arguments
		);
	...
	start: function() {
		...
		zerk.parent('tars.game.engine.system.tars').start.apply(
			this,
			arguments
		);
	...
	stop: function() {
		...
		zerk.parent('tars.game.engine.system.tars').stop.apply(
			this,
			arguments
		);
	...
	update: function() {
		...
		zerk.parent('tars.game.engine.system.tars').update.apply();
		
		for (var i=0;i<this._entities.length;i++) {
			
			if (!this._entities[i].components.tars.enableControl) return;
	...
	_onKeyDown: function(event) {
		...
		for (var i=0;i<this._entities.length;i++) {
			...
			var componentState=this._entities[i].components.tars;
	...
	_onContactBegin: function(sourceInfo,targetInfo) {
		...
		if (typeof sourceInfo.entity.components.tars=='undefined'
		&& typeof targetInfo.entity.components.tars=='undefined') {
			return true;
		}
		
		var info=((typeof sourceInfo.entity.components.tars!='undefined')
			? sourceInfo
			: targetInfo);
		
		var entity=info.entity;
		
		var componentState=entity.components.tars;
